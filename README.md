...
- ローカルで開く場合（Windows CMD）：
  ...
  Chrome または Edge で <http://localhost:8000/> を開いてください。`0x1d` 以降の JP ファームウェアでは接続時に画面モデルと設定を読み、カレンダー／時計モードでデバイス時刻が 60 秒を超えてずれていれば自動同期して画面を更新します。ファームウェアを書き換えても電子ペーパーは前の画像を保持するため、Web UI のファームウェアバージョンを確認してください。
...
`0x1d` では、この UC8176 3 色機向け nRF52811 初期モデルを `03` に修正しました。既存の保存設定がある場合はそのモデル値を優先し、Web UI は接続時に従来の `01`／`22` コマンドで読み込みます。Web UI はモデルを推測して書き換えません。
